...
if [ -x "./.orch/scripts/audit/check-adherence.sh" ]; then
  ./.orch/scripts/audit/check-adherence.sh "$SESSION_FILE" 2>/dev/null || true
fi

# Estimate tokens for the session
if [ -x "./.orch/scripts/audit/estimate-tokens.py" ] && command -v python3 &>/dev/null; then
  python3 ./.orch/scripts/audit/estimate-tokens.py "$SESSION_FILE" 2>/dev/null || true
fi
