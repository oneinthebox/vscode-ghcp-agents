...
### 5.3 Boundaries (`.orch/audit/config/boundaries.yaml`)

Defines which tools each agent can use and which files it can touch:

- **Coordinator:** read-only (`codebase` tool, reads config/package.json only)
...
> **Prerequisite:** Your workflow references skills by name. Those skills must exist as directories in `.github/skills/`. See [Section 12.1](#121-create-a-new-skill-step-by-step) for creating new skills that your workflow can reference.
...
> **Note:** Reports are richer when reference docs are populated. Scan skills read from `.orch/references/` for compatibility matrices, migration guides, and anti-pattern checklists. See [Section 12.2](#122-add-or-update-reference-docs) to set up reference docs.
...
> **Prerequisites:** HDS and Elevate skills need reference docs to operate at full effectiveness. Run `@docs /docs-fetch --origin <your-hds-storybook-url> --format html` to populate `.orch/references/internal/hds/`. See [Section 12.2](#122-add-or-update-reference-docs) for details.
...
## 12. Creating and Updating Skills

### 12.1 Create a New Skill (Step by Step)
...
> **Important:** If your SKILL.md declares `references:`, those docs MUST exist before the skill works. Complete this step BEFORE testing (Step 7). See [Section 12.2](#122-add-or-update-reference-docs) for the full reference doc lifecycle.
...
### 12.2 Add or Update Reference Docs
...
### 12.4 Updating an Existing Skill
